...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:                                                                   # Check to receive first
    ...
    while True:
        ...
        if not len(local_buffer) or not len(remote_buffer):                             # Close the sockets when there's no data to send
            client_socket.close()
            remote_socket.close()
...
def main():
    if len(sys.argv[1:]) != 5:
        ...
        print("Example: ./proxy.py 127.0.0.1 9000 10.12.10.100 9000 True")
        sys.exit(0)
